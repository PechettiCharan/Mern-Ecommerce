...
app.post("/signup", async (req, res) => {
  try {
    ...
    if (check) {
      return res
        .status(400)
        .json({ success: false, errrors: "existing user found" });
    }
...
const fetchUser = async (req, res, next) => {
  ...
  if (!token) {
    return res
      .status(401)
      .send({ errros: "Pleade authenticate using correct credentials" });
  } else {
    try {
      ...
      return res
        .status(401)
        .send({ errors: "Pleade authenticate using correct credentials" });
    }
  }
};
